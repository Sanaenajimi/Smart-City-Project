...
print("=" * 70)
print(f"⏱️  Intervalle : {INTERVAL_MINUTES} minute(s)")
print(f"🌍 Ville : {CITY}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['status'] == 'ok':
                ...
                print(f"  🌡️  Polluants collectés:")
                if pm25: print(f"     • PM2.5: {pm25} µg/m³")
                if pm10: print(f"     • PM10: {pm10} µg/m³")
                if no2: print(f"     • NO2: {no2} µg/m³")
                if o3: print(f"     • O3: {o3} µg/m³")
                if so2: print(f"     • SO2: {so2} µg/m³")
                if co: print(f"     • CO: {co} µg/m³")
                if nh3: print(f"     • NH3: {nh3} µg/m³")
...
def collect_weather():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  🌤️  Météo collectée:")
            print(f"     • Température: {temperature}°C (ressenti: {feels_like}°C)")
            print(f"     • Humidité: {humidity}%")
            print(f"     • Pression: {pressure} hPa")
            print(f"     • Vent: {wind_speed} m/s, direction: {wind_direction}°")
            print(f"     • Nuages: {clouds}%")
            print(f"     • Visibilité: {visibility}m")
...
def simulate_iot():
    ...
    for sensor_id, location, lat, lon in sensors:
        try:
            ...
            print(f"  ⚠️  Erreur capteur {sensor_id}: {str(e)}")
            pass
...
def collect_once():
    ...
    if weather_ok:
        print(f"  ✅ Météo complète collectée")
    
    time.sleep(2)
    
    # 3. Capteurs IoT
    iot_count = simulate_iot()
    print(f"  ✅ IoT - {iot_count} capteurs simulés avec tous les polluants")
    ...
    print(f"     • Météo: {total_weather} enregistrements")
...
def main():
    print("\n✅ Collecteur prêt à démarrer")
    print("📋 Collecte complète de :")
    print("   • Qualité de l'air : PM2.5, PM10, NO2, O3, SO2, CO, NH3")
    print("   • Météo : Temp, Humidité, Vent, Pression, Visibilité, Nuages")
    print("   • Capteurs IoT : 3 zones simulées")
    print("   • Alertes : Génération automatique\n")
    
    # Première collecte
    print("🚀 Première collecte immédiate...")
    collect_once()
    
    # Programmer les collectes
    schedule.every(INTERVAL_MINUTES).minutes.do(collect_once)
    
    print(f"\n🔄 Collecte automatique programmée toutes les {INTERVAL_MINUTES} minute(s)")
    print("💡 Le dashboard se mettra à jour automatiquement")
